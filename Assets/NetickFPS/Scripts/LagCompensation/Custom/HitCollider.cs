...
// TODO make new namespace for this.
namespace PG.LagCompensation
{
    ...
    public abstract class HitCollider : NetworkBehaviour
    {
        ...
        [Header("Store history settings")]
        public int StoreHistoryInSeconds = 3;
        private int MaxTicksToStore;
        ...
        // Changes to use network start/update/destroy
        ...
        {
            float seconds = StoreHistoryInSeconds;

            MaxTicksToStore = Mathf.CeilToInt(seconds * Sandbox.Config.TickRate);
        }
	    ...
	    public void ORIGINALAddFrame(double _localTime)
        {
            ...
            // the frame times no longer has time, it has ticks which are ints.
            //FrameTimes.Add(_localTime);
        }

        /// <summary>
        /// Call this every tick after all movement updates!, Records state of this this collider and adds it to the list
        /// </summary>
        /// <param name="tick"></param>
        public void AddState(int tick)
        {
            // TODO use queue instead for more performance, remove oldest stored frame in list.
            if (FrameTicks.Count >= MaxTicksToStore)
            {
                FrameTicks.RemoveAt(0);
                FrameData.RemoveAt(0);
            }

            // Add state pos rot and time/tick.

            FrameData.Add(new TransformFrameData(transform.position, transform.rotation));
            FrameTicks.Add(tick);
        }
        ...
        public void ORIGINALCacheInterpolationPositionRotation(double simulationTime)
        {
            for (int i = FrameTicks.Count - 1; i >= 0; i--)
            {
                if (FrameTicks[i] <= simulationTime) // if the data at [i] is older than the desired simulation time
                {
                    if (i < FrameTicks.Count - 1) // if there is a newer frame
                    {
                        double fraction = Math.Clamp((simulationTime - FrameTicks[i]) / (FrameTicks[i + 1] - FrameTicks[i]), 0d, 1d);
