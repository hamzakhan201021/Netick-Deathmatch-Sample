...
{
    [CreateAssetMenu(fileName = "LiteNetLibTransportProviderLatency", menuName = "Netick/Transport/LiteNetLibTransportProviderLatency", order = 1)]
    public class LNLTransportProviderLatency : NetworkTransportProvider
    {
        public override NetworkTransport MakeTransportInstance() => transport = new LNLTransportLatency();

        public static LNLTransportLatency transport;
    ...
    public class LNLTransportLatency : NetworkTransport, INetEventListener
    {
        public class LNLConnectionLatency : TransportConnection
        {
            public LNLTransportLatency Transport;
            ...
            public LNLConnectionLatency(LNLTransportLatency transport)
            {
                Transport = transport;
            }
        ...
        private Dictionary<NetPeer, LNLConnectionLatency> _clients = new Dictionary<NetPeer, LNLConnectionLatency>();
        private Queue<LNLConnectionLatency> _freeClients = new Queue<LNLConnectionLatency>();
        ...
        public override void Init()
        {
            ...
            for (int i = 0; i < Engine.Config.MaxPlayers; i++)
                _freeClients.Enqueue(new LNLConnectionLatency(this));
        }
...
#if UNITY_EDITOR
            LatencyDebug.TransportInitialized(_isServer);
#endif
            _port = port;
        }
        ...
        {
            _netManager.DisconnectPeer(((LNLConnectionLatency)connection).LNLPeer);
        }
