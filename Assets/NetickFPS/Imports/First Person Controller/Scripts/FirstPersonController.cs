...
public class FirstPersonController : NetworkedCharacterController
{
    ...
    public override void NetworkUpdate()
    {
        ...
        // Replaced to implement/support continous jumping
        networkInput.JumpInput |= Input.GetKeyDown(KeyCode.Space);
        //networkInput.JumpInput = Input.GetKey(KeyCode.Space);

        if (ContinousJumping) networkInput.JumpInput = Input.GetKey(KeyCode.Space);
        else networkInput.JumpInput = Input.GetKeyDown(KeyCode.Space);
    ...
    public override void NetworkFixedUpdate()
    {
        ...
        if (Sandbox.IsServer || IsPredicted)
        {
            ...
            _velocity.y += GravityMultiplier * GravityAcceleration * Sandbox.FixedDeltaTime;
